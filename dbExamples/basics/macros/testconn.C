...
//	cout << "-I- MySQL server connected  server:# " << serv <<  endl;
  ...
  TSQLServer *serv = TSQLServer::Connect("mysql://127.0.0.1:4000/cbm", "root", "");
  if (serv!=0 && serv->IsConnected()){
	cout << "-I- tiDB server connected  server:# " << serv <<  endl;

// Statement 

   TSQLStatement* stmt = NULL; 
   stmt = serv->Statement("CREATE TABLE TESTTABLE2 (ID1 INT, ID2 INT, ID3 BLOB)");    
