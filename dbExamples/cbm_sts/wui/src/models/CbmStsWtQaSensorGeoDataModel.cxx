...
WString CbmStsQaSensorGeoDataModel::dateDisplayFormat(WString::fromUTF8("MMM dd, yyyy"));
WString CbmStsQaSensorGeoDataModel::dateEditFormat(WString::fromUTF8("dd-MM-yyyy"));


CbmStsQaSensorGeoDataModel::CbmStsQaSensorGeoDataModel(int rows, int columns, WObject *parent)
...
int CbmStsQaSensorGeoDataModel::rowCount(const Wt::WModelIndex& parent) const
{
    if (!parent.isValid())
      return rows_;
    else
...
int CbmStsQaSensorGeoDataModel::columnCount(const Wt::WModelIndex& parent ) const
{
    if (!parent.isValid())
      return columns_;
    else
...
boost::any CbmStsQaSensorGeoDataModel::data(const Wt::WModelIndex& index, int role) const
{
...
boost::any CbmStsQaSensorGeoDataModel::headerData(int section, Wt::Orientation orientation, int role) const
{
...
void CbmStsQaSensorGeoDataModel::fillheader()
{
...
void CbmStsQaSensorGeoDataModel::fillrows(int rid)
{
...
// if rid = 0 take the last version
 ...
 // Now get all rows from parameter
  FairDbReader<CbmStsDbQaGeometryPar>* pReader = pQaSensorPar->GetParamReader();
  int numRows = pReader->GetNumRows();
  // Update the n_rows in model
  rows_ = 0;
  for (int i = 0; i < numRows; ++i)
    {
      CbmStsDbQaGeometryPar* cgd = (CbmStsDbQaGeometryPar*) pReader->GetRow(i);
      if (!cgd) { continue; }
       ...
       double pitch = cgd->GetPitch();
       double stereo_p =  cgd->GetStereoAngleP();
       double stereo_n =  cgd->GetStereoAngleN();
