...
CbmStsWtQaIvTableChartView::CbmStsWtQaIvTableChartView(Wt::WContainerWidget *parent):
  WContainerWidget(parent)
{
  ...
  new CbmStsWtQaIvChartConfig(chart, this);
}
...
CbmStsWtQaIvChartView::CbmStsWtQaIvChartView(int sid, WContainerWidget *parent):
  WContainerWidget(parent)
  ,fInit(false)
  ,fName("")
  ,fUID(0)
...
// Set editing properties
 for (int row = 0; row < model->rowCount(); ++row){
  ...
  CbmStsWtQaIvChartConfig *config = new CbmStsWtQaIvChartConfig(chart, this);
...
void CbmStsWtQaIvChartView::initIvContainer( int sid, int rid)
{
  // DB! get container using reference !
  const CbmStsDbQa& stsQa = CbmStsWtQaDataObjects::instance().getStsQa();    
  int nIv = stsQa.GetNumIvMeas();
  TObjArray* sArr = stsQa.GetListOfIvMeas();
  
  // Get Iv Meas corresponding to sId
  CbmStsDbQaIvNewPar* cgd = (CbmStsDbQaIvNewPar*) sArr->At(sid); 
  if (cgd)
    {
