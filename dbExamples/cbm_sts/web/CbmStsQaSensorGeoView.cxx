...
CbmStsQaSensorGeoView::CbmStsQaSensorGeoView(WContainerWidget *parent, string pName)
...
cout << "-I- CbmStsQaSensorGeoView::  Table View name " << t_meta.TableName()
...
CbmStsQaSensorGeoDataModel *t_model =
                         new CbmStsQaSensorGeoDataModel(100, n_cols-3, fTableView);
...
fTableView->clicked().connect(
  boost::bind(&CbmStsQaSensorGeoView::editFile, this, _1, _2));
  //this, &CbmStsQaSensorGeoView::editFile);

fTableView->doubleClicked().connect(
  boost::bind(&CbmStsQaSensorGeoView::editFiled, this, _1, t_model));

// Events Notify
fTableView->mouseWentDown().connect(
    boost::bind(&CbmStsQaSensorGeoView::showPopup, this, _1, _2));
...
CbmStsQaSensorGeoView::CbmStsQaSensorGeoView(WContainerWidget *parent, string pName, vector<int>& sid)
  : WContainerWidget(parent)
{
// Set version of Data Model
...
if (!FairDbWtParFactory::Instance().GetParameter<CbmStsDbQaGeometryPar>()){
  auto pQaSensorGeoPar =
  FairDbWtParFactory::Instance().InitContainer<CbmStsDbQaGeometryPar>();
  if(pQaSensorGeoPar)
  {
    FairDbReader<CbmStsDbQaGeometryPar>* r_Sensor = pQaSensorGeoPar->GetParamReader();
    FairDbWtParFactory::Instance().RegisterReader<CbmStsDbQaGeometryPar>(r_Sensor);
  }
}
cout <<"-I- CbmStsDbQaSensorNewPar is initialised " << endl;

cout << "-I- CbmStsQaSensorGeoView getParInfo" << endl;
FairDbWtParFactory::parInfo p_Info = FairDbWtParFactory::Instance().GetParInfo(pName);
FairDbTableMetaData t_meta = p_Info.fTableInterface->GetMetaData();
...
CbmStsQaSensorGeoView::~CbmStsQaSensorGeoView()
{
 cout << "~CbmStsQaSensorGeoView " << endl;
}



void CbmStsQaSensorGeoView::editFile(const WModelIndex& item, const WMouseEvent& event)
{

 cout << "-I- CbmStsQaSensorGeoView:: single clicked node :: item  row# "
 ...
 //cout << "-I- CbmStsQaSensorGeoView: Selected Id : " << fSensorId  << endl;

}

void CbmStsQaSensorGeoView::editFiled(const WModelIndex& item,
                                          WAbstractItemModel* model)
{
 cout << "-I- CbmStsQaSensorGeoView:: doubleClicked :: item  row# " << item.row()
      <<  " column = " << item.column()
      << " model " << model
      << endl;
...
void CbmStsQaSensorGeoView::showPopup(const WModelIndex& item,
                                        const WMouseEvent& event )
{
    cout << "-I- CbmStsQaSensorGeoView:: showpopup() :: item  row# "
       << item.row()
      <<  " column = " << item.column()
     << endl;
    ...
    //if (fPopup) {delete fPopup; fPopup=NULL;}
      ...
      fPopup->aboutToHide().connect(this, &CbmStsQaSensorGeoView::popupAction);
...
void CbmStsQaSensorGeoView::popupAction()
{
   ...
   fPopupActionBox = new WMessageBox(str,"Actions: " + text, NoIcon, Ok);
      if (text=="Close")
          fPopupActionBox->buttonClicked().connect(this, &CbmStsQaSensorGeoView::dialogDone);
      else if (text=="Show IV")
          fPopupActionBox->buttonClicked().connect(
            boost::bind(&CbmStsQaSensorGeoView::displayIV, this, fTableView->selectedIndexes()));
...
void CbmStsQaSensorGeoView::displayIV(const WModelIndexSet& aSet)
{
...
void CbmStsQaSensorGeoView::dialogDone()
{
