...
CbmStsQaSensorResultsView::CbmStsQaSensorResultsView(WContainerWidget *parent, string pName)
...
cout << "-I- CbmStsQaSensorResultsView::  Table View name " << t_meta.TableName()
...
CbmStsQaSensorResultsDataModel *t_model =
                         new CbmStsQaSensorResultsDataModel(50, n_cols-3, fTableView);
...
fTableView->clicked().connect(
  boost::bind(&CbmStsQaSensorResultsView::editFile, this, _1, _2));
  //this, &CbmStsQaSensorResultsView::editFile);

fTableView->doubleClicked().connect(
  boost::bind(&CbmStsQaSensorResultsView::editFiled, this, _1, t_model));

// Events Notify
fTableView->mouseWentDown().connect(
    boost::bind(&CbmStsQaSensorResultsView::showPopup, this, _1, _2));
...
CbmStsQaSensorResultsView::CbmStsQaSensorResultsView(WContainerWidget *parent, string pName, vector<int>& sid)
  : WContainerWidget(parent)
{
// Set version of Data Model
...
if (!FairDbWtParFactory::Instance().GetParameter<CbmStsDbQaResultsPar>()){
  auto pQaSensorResultsPar =
  FairDbWtParFactory::Instance().InitContainer<CbmStsDbQaResultsPar>();
  if(pQaSensorResultsPar)
  {
    FairDbReader<CbmStsDbQaResultsPar>* r_Sensor = pQaSensorResultsPar->GetParamReader();
    FairDbWtParFactory::Instance().RegisterReader<CbmStsDbQaResultsPar>(r_Sensor);
  }
  cout << "-I- CbmStsQaSensorResultsView ResultsPar initialised !" << endl;
}
...
CbmStsQaSensorResultsView::~CbmStsQaSensorResultsView()
{
 cout << "~CbmStsQaSensorResultsView " << endl;
}



void CbmStsQaSensorResultsView::editFile(const WModelIndex& item, const WMouseEvent& event)
{

 cout << "-I- CbmStsQaSensorResultsView:: single clicked node :: item  row# "
 ...
 //cout << "-I- CbmStsQaSensorResultsView: Selected Id : " << fSensorId  << endl;

}

void CbmStsQaSensorResultsView::editFiled(const WModelIndex& item,
                                          WAbstractItemModel* model)
{
 cout << "-I- CbmStsQaSensorResultsView:: doubleClicked :: item  row# " << item.row()
      <<  " column = " << item.column()
      << " model " << model
      << endl;
...
void CbmStsQaSensorResultsView::showPopup(const WModelIndex& item,
                                        const WMouseEvent& event )
{
    cout << "-I- CbmStsQaSensorResultsView:: showpopup() :: item  row# "
       << item.row()
      <<  " column = " << item.column()
     << endl;
    ...
    //if (fPopup) {delete fPopup; fPopup=NULL;}
      ...
      fPopup->aboutToHide().connect(this, &CbmStsQaSensorResultsView::popupAction);
...
void CbmStsQaSensorResultsView::popupAction()
{
   ...
   fPopupActionBox = new WMessageBox(str,"Actions: " + text, NoIcon, Ok);
      if (text=="Close")
          fPopupActionBox->buttonClicked().connect(this, &CbmStsQaSensorResultsView::dialogDone);
      else if (text=="Show IV")
          fPopupActionBox->buttonClicked().connect(
            boost::bind(&CbmStsQaSensorResultsView::displayIV, this, fTableView->selectedIndexes()));
...
void CbmStsQaSensorResultsView::displayIV(const WModelIndexSet& aSet)
{
...
void CbmStsQaSensorResultsView::dialogDone()
{
